...
| 8 | [`WebApiDemo`](WebApiDemo) | ASP.NET Core Minimal API wired through `PostQuantum.Hybrid.AspNetCore`'s DI extensions. GET the public keys, POST a plaintext to `/seal`, POST data to `/sign`. **Swagger UI at the root** for click-through exploration. Containerized — see the [`Dockerfile`](WebApiDemo/Dockerfile) for local runs and [`DEPLOY.md`](WebApiDemo/DEPLOY.md) for the Azure Container Apps deploy. | When you're integrating into an ASP.NET service with static keys. |
